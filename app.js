...
let serverRecords = [];
function registry() { return serverRecords; }
async function api(action, body, query = {}) {
  const response = await fetch(`/api/registry?${new URLSearchParams({action,...query})}`, {method:body ? 'POST':'GET', credentials:'same-origin', headers:body ? {'Content-Type':'application/json'}:{}, body:body ? JSON.stringify(body):undefined});
  let data; try { data = await response.json(); } catch { throw new Error('Online certificate service is unavailable. Please contact the institute.'); }
  if (!response.ok) { if (response.status === 401 && action !== 'login') { state.admin = false; serverRecords = []; state.selected = null; updateAccessUI(); } throw new Error(data.error || 'Unable to complete this request.'); }
  return data;
}
async function refreshRegistry() { serverRecords = (await api('list')).records; renderRegistry(); }
async function restoreSession() { try { state.admin = (await api('session')).authenticated; if (state.admin) await refreshRegistry(); } catch (error) { $('#adminLoginStatus').textContent = error.message; } updateAccessUI(); }
...
function showCourse(id) {
  ...
  $('#courseDetail').innerHTML = `<p class="detail-tag">${escapeHTML(course.cat)}</p><h2 class="detail-title" id="dialogTitle">${escapeHTML(course.title)}</h2><p class="detail-description">${escapeHTML(course.desc)}</p><div class="detail-meta"><span>Suggested: ${course.months} months</span><span>${escapeHTML(course.level)}</span><span>4 learning modules</span></div><h3 class="detail-heading">Who can join?</h3><p>${escapeHTML(detail.entry)}</p><h3 class="detail-heading">Tools you’ll practise with</h3><p>${escapeHTML(detail.tools)}</p><h3 class="detail-heading">Your learning plan</h3><div class="module-list">${course.modules.map((module,i) => `<details ${i === 0 ? 'open' : ''}><summary><span>0${i+1}</span> ${escapeHTML(module)}</summary><p>${escapeHTML(detail.lessons[i])}</p></details>`).join('')}</div><h3 class="detail-heading">By the end, you can</h3><ul class="detail-list">${detail.outcomes.map(item => `<li>${escapeHTML(item)}</li>`).join('')}</ul><div class="detail-project"><strong>YOUR PRACTICAL PROJECT</strong>${escapeHTML(course.project)}</div><h3 class="detail-heading">Practice, review and completion</h3><p>Proposed format: instructor demonstration, guided exercises, independent practice and a final project review. Completion criteria and attendance requirements must be confirmed with BIT before enrolment.</p><p class="detail-note">Proposed programme outline. Confirm fees, batch timings, software availability and final syllabus with the institute. BIT certificates are institute-issued; no external accreditation or employment guarantee is implied.</p><div class="dialog-actions"><button class="button" data-outline="${escapeHTML(course.id)}">Download full outline ↓</button><button class="button button-light" data-cert="${escapeHTML(course.id)}">Admin certificate studio ↗</button></div>`;
...
function downloadOutline(id) {
  const c = courses.find(item => item.id === id); if (!c) return; const d = courseDetails[id];
  const text = ['BHARAT INSTITUTE OF TECHNOLOGY', c.title, `Proposed duration: ${c.months} months | Level: ${c.level}`, c.desc, 'ENTRY GUIDANCE', d.entry, 'TOOLS', d.tools, 'LEARNING PLAN', ...c.modules.map((m,i) => `${i+1}. ${m}\n${d.lessons[i]}`), 'LEARNING OUTCOMES', ...d.outcomes, 'FINAL PROJECT', c.project, 'ASSESSMENT', 'Guided exercises, independent practice and a final project review. Confirm completion and attendance requirements before enrolment.', 'ADMISSIONS', 'Confirm fees, batch dates, software availability and final syllabus with BIT. Proposed outline only; no external accreditation or job guarantee is implied.'].join('\n\n');
  saveBlob(new Blob([text],{type:'text/plain;charset=utf-8'}),`BIT-${id}-course-outline.txt`);
}
$('#closeDialog').addEventListener('click', () => dialog.close());
dialog.addEventListener('click', event => { if (event.target === dialog) dialog.close(); });

function decodePayload(value) { try { const text = value.replace(/-/g,'+').replace(/_/g,'/'); return JSON.parse(new TextDecoder().decode(Uint8Array.from(atob(text), c => c.charCodeAt(0)))); } catch { return null; } }
function verificationUrl(record) { return `${location.origin}/#verify?id=${encodeURIComponent(record.id)}`; }
function qrUrl(url) { const id = new URLSearchParams(new URL(url).hash.split('?')[1]).get('id'); return `/api/registry?action=qr&id=${encodeURIComponent(id)}`; }
...
function nextCertificateNumber() { return `BIT-${today.getFullYear()}-${crypto.randomUUID().slice(0,8).toUpperCase()}`; }
...
function updateCertificatePreview(record = state.selected) {
  ...
  const qr = $('#previewQr');
  if (record?.id) { qr.src = qrUrl(verificationUrl(record)); $('#qrNote').textContent = 'QR ready — open it to verify this certificate record.'; } else { qr.removeAttribute('src'); $('#qrNote').textContent = 'Create a record to generate its verification QR code.'; }
}
...
certificateForm.addEventListener('submit', async event => {
  ...
  const submit = certificateForm.querySelector('button[type="submit"]'); submit.disabled = true; submit.textContent = 'Registering certificate…';
  try {
    const {record} = await api('issue', {...data,id:data.number});
    serverRecords = [record,...records]; state.selected = record; updateCertificatePreview(record); renderRegistry();
    $('#certificateStatus').textContent = `Certificate ${record.id} registered online. Its QR code can now be verified on another device.`;
    certificateForm.elements.number.value = nextCertificateNumber();
  } catch (error) { $('#certificateStatus').textContent = error.message; }
  finally { submit.disabled = false; submit.innerHTML = 'Create certificate <span>↗</span>'; }

});
$('#printCertificate').addEventListener('click', () => { if (!state.admin) return; if (!state.selected) { $('#certificateStatus').textContent = 'Create the certificate record first, then print or export it.'; return; } const qr = $('#previewQr'); if (!qr.complete || !qr.naturalWidth) { $('#certificateStatus').textContent = 'Wait for the QR image to load before printing.'; return; } window.print(); });
...
async function exportCertificate(record) {
  ...
  const image = await loadImage(qrUrl(verificationUrl(record))); context.drawImage(image,1090,1180,300,300);
...
function renderRegistry() {
  ...
  $('#certificateRegister').innerHTML = filtered.map(record => `<article class="register-row"><div><strong>${escapeHTML(record.student)}</strong><small>${escapeHTML(record.course)}</small></div><div><strong>${escapeHTML(record.id)}</strong><small>${escapeHTML(readableDate(record.date))}</small></div><span class="register-status ${record.revoked ? 'revoked' : 'active'}">${record.revoked ? 'Revoked' : 'Active'}</span><span>${escapeHTML(record.signatory)}</span><div class="register-actions"><button class="icon-button" data-registry-action="open" data-id="${escapeHTML(record.id)}">Export</button><button class="icon-button" data-registry-action="verify" data-id="${escapeHTML(record.id)}" aria-label="Open verification for ${escapeHTML(record.id)}">View</button><button class="icon-button" data-registry-action="copy" data-id="${escapeHTML(record.id)}" aria-label="Copy verification link for ${escapeHTML(record.id)}">Copy</button><button class="icon-button danger" data-registry-action="revoke" data-id="${escapeHTML(record.id)}" aria-label="${record.revoked ? 'Restore' : 'Revoke'} ${escapeHTML(record.id)}">${record.revoked ? 'Restore' : 'Revoke'}</button></div></article>`).join('');
}
$('#registrySearch').addEventListener('input', renderRegistry);

function updateAccessUI() { $('#certificateGate').hidden = state.admin; $('#certificateWorkspace').hidden = !state.admin; $('#adminLogin').hidden = state.admin; $('#adminDashboard').hidden = !state.admin; if (state.admin) renderRegistry(); }
$('#adminLoginForm').addEventListener('submit', async event => {
  event.preventDefault(); const input = $('#adminPassword'); const status = $('#adminLoginStatus'); const button = event.currentTarget.querySelector('button'); button.disabled = true; status.textContent = 'Checking access…';
  try { await api('login',{password:input.value}); state.admin = true; input.value = ''; await refreshRegistry(); status.textContent = 'Signed in.'; updateAccessUI(); }
  catch (error) { status.textContent = error.message; } finally { button.disabled = false; }
});
$('#adminLogout').addEventListener('click', async () => { try { await api('logout',{}); state.admin=false; serverRecords=[]; state.selected=null; updateAccessUI(); location.hash='admin'; $('#adminLoginStatus').textContent='Signed out.'; } catch(error) { $('#registryStatus').textContent=error.message; } });
$('#downloadCertificate').addEventListener('click', async event => { if (!state.admin || !state.selected) { $('#certificateStatus').textContent = 'Create or open a registered certificate first.'; return; } event.currentTarget.disabled=true; try { await exportCertificate(state.selected); $('#certificateStatus').textContent='PNG export ready.'; } catch { $('#certificateStatus').textContent='QR image could not load. Export stopped; please try again.'; } finally { $('#downloadCertificate').disabled=false; } });
function renderVerification(result, error) {
  const target = $('#verifyResult');
  if (!result) { target.className='verify-result invalid'; target.innerHTML=`<div class="result-mark">!</div><h2>Unable to verify</h2><p>${escapeHTML(error || 'Enter a certificate number or complete verification link.')}</p>`; return; }
  const revoked = result.revoked; const record = result.record;
  target.className=`verify-result ${revoked ? 'revoked':'valid'}`;
  target.innerHTML=`<div class="result-mark">${revoked ? '!':'✓'}</div><h2>${revoked ? 'Certificate revoked':'Certificate verified'}</h2><p>${revoked ? 'BIT has revoked this certificate. It is no longer valid.':'This certificate is active in the BIT online register. Compare these details with the printed certificate.'}</p><div class="verify-data">${[['STUDENT',record.student],['CERTIFICATE NO.',record.id],['COURSE',record.course],['COMPLETED',readableDate(record.date)],['SIGNATORY',record.signatory],['REGISTER STATUS',revoked ? 'Revoked':'Active']].map(([label,value])=>`<div><span>${label}</span><strong>${escapeHTML(value)}</strong></div>`).join('')}</div><p class="form-note">Checked just now against the institute register. Institute-issued certificate; this result does not imply external accreditation.</p>`;
}
let verifyRequest = 0;
function verificationId(value) {
  if (/^[a-z0-9-]+$/i.test(value) && value.toUpperCase().startsWith('BIT-')) return value;
  try { const query = new URLSearchParams(new URL(value,location.href).hash.split('?')[1]); if(query.get('id')) return query.get('id'); if(query.get('data')) return decodePayload(query.get('data'))?.id; } catch {}
  return decodePayload(value)?.id || value;
}
async function verifyNumber(value) {
  const request = ++verifyRequest; const target = $('#verifyResult'); target.className='verify-result neutral'; target.textContent='Checking the online register…';
  try { const result=await api('verify',undefined,{id:verificationId(value)}); if(request === verifyRequest) renderVerification(result); }
  catch(error) { if(request === verifyRequest) renderVerification(null,error.message); }
}
function handleVerificationQuery() { const query=routeQuery(); const id=query.get('id') || decodePayload(query.get('data') || '')?.id; if(id) { $('#verifyInput').value=id; verifyNumber(id); } }
$('#verifyForm').addEventListener('submit', event => { event.preventDefault(); verifyNumber($('#verifyInput').value.trim()); });
document.addEventListener('click', async event => {
  ...
  const certButton=event.target.closest('[data-cert]'); if(certButton) { dialog.close(); $('#certificateCourse').value=certButton.dataset.cert; editCertificateForm(); location.hash=state.admin ? 'certificates':'admin'; }
  const button=event.target.closest('[data-registry-action]'); if(!button) return;
  const record=registry().find(item=>item.id===button.dataset.id); if(!record) return;
  const action=button.dataset.registryAction;
  if(action==='verify') { location.hash=`verify?id=${encodeURIComponent(record.id)}`; return; }
  if(action==='open') { for(const name of ['student','date','signatory','theme']) certificateForm.elements[name].value=record[name]; certificateForm.elements.number.value=record.id; const course=courses.find(c=>c.title===record.course); if(course) certificateForm.elements.course.value=course.id; state.selected=record; updateCertificatePreview(record); location.hash='certificates'; return; }
  if(action==='copy') { try { await navigator.clipboard.writeText(verificationUrl(record)); button.textContent='Copied'; } catch { $('#registryStatus').textContent=verificationUrl(record); } return; }
  if(action==='revoke') { button.disabled=true; try { const result=await api('status',{id:record.id,revoked:!record.revoked}); serverRecords=serverRecords.map(item=>item.id===record.id ? result.record:item); if(state.selected?.id===record.id) state.selected=result.record; renderRegistry(); $('#registryStatus').textContent=`${record.id}: ${result.record.revoked ? 'revoked':'restored'}.`; } catch(error) { $('#registryStatus').textContent=error.message; button.disabled=false; } }
});
